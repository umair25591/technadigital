...
/* ==========================================================================
   ...
   ==========================================================================
   Single-open accordion. On wide viewports (.can-morph) the clicked card's
   flex-grow is tweened up with an elastic ease — an overshoot-then-settle
   "stretch" — while its siblings' flex-grow tweens down, so the row visibly
   redistributes width around the open card. Narrower viewports skip the
   width morph entirely and just expand the description in place.
   ========================================================================== */
(function initRiskAccordion() {
  ...
  const OPEN_GROW = 2.6;
  const CLOSED_GROW = 0.85;

  const wideLayout = window.matchMedia('(min-width: 1025px)');
  let openItem = null;

  function animateGrow(item, value, ease) {
    gsap.to(item, {
      flexGrow: value,
      duration: 0.8,
  ...
  function setOpen(item) {
    const morph = grid.classList.contains('can-morph');

    items.forEach((other) => {
      const toggle = other.querySelector('.risk-item__toggle');
      const isTarget = other === item;
      other.classList.toggle('is-open', isTarget);
      toggle.setAttribute('aria-expanded', String(isTarget));

      if (morph) {
        // The opening card grows smoothly (no overshoot); its shrinking
        // siblings keep the elastic "stretch" for the springy contrast.
        animateGrow(other, isTarget ? OPEN_GROW : CLOSED_GROW, isTarget ? 'power3.out' : undefined);
      }
    });
  ...
  function setClosed() {
    ...
    items.forEach((other) => {
      ...
      if (morph) {
        // The card that was open shrinks smoothly; the rest ease back up.
        animateGrow(other, 1, other === wasOpen ? 'power3.out' : undefined);
      }
    });
  ...
  // Crossing the desktop breakpoint mid-interaction — reset to a clean,
  // equal-width state rather than leaving stale inline flex-grow values.
  function syncLayout() {
    grid.classList.toggle('can-morph', wideLayout.matches);
    items.forEach((item) => { item.style.flexGrow = ''; });
    setClosed();
  }
